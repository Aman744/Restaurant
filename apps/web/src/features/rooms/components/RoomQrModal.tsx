...
export const RoomQrModal: React.FC<RoomQrModalProps> = ({ tenantId, room, onClose }) => {
  const toast = useToast();
  const [qrDataUrl, setQrDataUrl] = useState<string>('');

  const baseUrl = `${window.location.origin}${window.location.pathname}`.replace(/\/$/, '');
  const targetUrl = room
    ? `${baseUrl}/#/customer/room/${tenantId}/${room.id}`
    : '';
  ...
  const handleDownload = () => {
    if (!qrDataUrl) return;
    const filename = `qr_room_${room.roomNumber.toLowerCase().replace(/\s+/g, '_')}.png`;
    ...
    toast.success(`Downloaded high-res scannable QR code for Room ${room.roomName || room.roomNumber}!`);
  };

  const handleCopyLink = () => {
    navigator.clipboard.writeText(targetUrl);
    toast.success('Copied room order URL to clipboard!');
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-sm border border-zinc-800 bg-zinc-950 p-6 shadow-2xl rounded-3xl text-white text-center space-y-5 relative">
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-center gap-2">
            ...
            <h3 className="text-lg font-extrabold text-white">
              {(room.roomName || room.roomNumber).toLowerCase().startsWith('room') 
        ...
        <div className="p-5 bg-white border border-zinc-200 rounded-2xl flex flex-col items-center justify-center shadow-xl space-y-2">
          {qrDataUrl ? (
            <img src={qrDataUrl} alt={`QR Code for Room ${room.roomNumber}`} className="w-56 h-56 object-contain" />
          ) : (
          ...
          <span className="text-[10px] font-extrabold uppercase tracking-widest text-zinc-800">
            {room.roomNumber.toLowerCase().startsWith('room') 
              ? room.roomNumber 
              : `Room ${room.roomNumber}`} • Scan to Order
          </span>
        </div>
